import type { PhotoItem, PlatformPreset, WatermarkSettings } from '../types';
...
export async function processAndSave(
  ...
  for (let i = 0; i < photos.length; i++) {
    ...
    try {
      const { base64 } = await exportOne(p.uri, settings, settings.logoUri, preset);
